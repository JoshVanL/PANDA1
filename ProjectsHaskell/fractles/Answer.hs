...
run :: Int -> Command -> Command
run 0 _ = error "Can't run 0 times."
run 1 c = c
run n c = c :> run (n-1) c
...
-- TODO #1.2
polygon :: Int -> Command
polygon n = run n (Fd 1 :> Rt (360 / fromIntegral n))
...
-- TODO #1.3
squares :: Int -> Command
squares 0 = error "Can't run 0 times."
squares 1 = square
...
repscale 0 _ _ = error "Can't run 0 times."
repscale 1 _ c = c
repscale 2 x c = c :> scale x c
repscale n x c = c :> scale x (repscale (n-1) x c)
...
drawTree d = foldTree tip node
  where
    ...
    node l x r = Go (Fd (fromIntegral x) :> Go(Lt d :> l) :> Go (Rt d :> r))
